    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources:
            ...
            # for image_file in image_files:
                ...
                print(f"[INFO] Processing image {idx+1}/{len(image_files)}: {image_file}") # img 파일이 잘 인식이 됐는지 확인
                ...
                img_tensor = get_image_info(image_file, self.image_min_pixel, self.image_max_pixel, self.image_resized_w, self.image_resized_h)
                
               
                if isinstance(img_tensor, torch.Tensor):
                    print(f"[SUCCESS] get_image_info returned tensor with shape: {img_tensor.shape}")
                elif hasattr(img_tensor, "size"):
                    print(f"[SUCCESS] get_image_info returned PIL image with size: {img_tensor.size}")
                else:
                    print(f"[WARN] get_image_info returned unknown type: {type(img_tensor)}")
